...
class Config:
    ...
    def __init__(self, max_length, embed_size, output_size, n_features =1 , n_classes=0, hidden_unit_size = 100, batch_size = 256, n_epochs = 10, num_layers =1, learning_rate=0.05, drop_out = 0.5, numpad=2):
...
class RNN_LSTM:
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, shape=(None, self.config.numpads+1)) #n_features should just be 1 here (the index into the embeddings) first None ==batch_size

        self.dropout_placeholder = tf.placeholder(tf.float32)

        self.labels_placeholder = tf.placeholder(tf.int32, shape= (None,)) #None here is batch_size
    ...
    def add_embeddings(self):
        ...
        embeddings = tf.reshape(embeddings, [-1, self.config.numpads+1, self.config.n_features * self.config.embed_size]) #1 here since we're only feeding one word at a time
        self.x = embeddings
    ...
    def training(self): #main training function for the model
        ...
        # return outputs
        return (outputs[:, outputs.get_shape()[1]-1,:])
    ...
    def train_on_batch_single_new(self, sess, batch, numpad):
        ...
        for post in batch:
            ...
            for idx in range(len(post)-numpad-1):

                batch_x.append(post[idx:idx+numpad+1])

                try:
                    batch_y.append(post[idx+numpad+2])
                except IndexError:

                    batch_y.append(post[idx+numpad+1])
        #make the batches into a matrix so that we can have easier time feeding
        ...
        feed = self.create_feed_dict(inputs_batch=batch_x_mat, labels_batch= batch_y_mat, dropout= self.config.drop_out)
    ...
    def run_epoch(self, sess, data, numpad):
...
def generate(args):
    ...
    c = Config(max_length = 1, embed_size = embeddings.shape[1], output_size=embeddings.shape[0], batch_size = 36, num_layers=args.numlayers, drop_out=1, hidden_unit_size=args.hiddensize,
               numpad = args.numpad) #max length is 1 becuase we want 1 word generated at a time
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            for sent in range(args.numsentences):
                ...
                x = [current_ind, current_ind, current_ind]
                ...
                while current_word != '<end>':
                    ...
                    print largest_10_unscaled_p
                    print [vocabs_reversed[ind] for ind in largest_10_inds]
...
if __name__ == '__main__':
    ...
    parse.add_argument('-np', '--numpad', type= int, default = 2)
