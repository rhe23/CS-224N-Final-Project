...
def generate_padded_seq(max_length, vocab_length, sentences):
    return np.array([sentence + [vocab_length-1]*(max_length-len(sentence)) for sentence in sentences])
...
class RNN_LSTM:
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, shape=(None, self.config.max_length)) #n_features should just be 1 here (the index into the embeddings)
        ...
        self.labels_placeholder = tf.placeholder(tf.int32, shape= (None, self.config.max_length-1))
    ...
    def test_session(self, session): #diagnostic for testing pieces of the model
        ...
        inds_y = [i[1:] for i in inds]
        small_s = generate_padded_seq(self.config.max_length, self.config.output_size, inds)
        small_s_y = generate_padded_seq(self.config.max_length-1, self.config.output_size, inds_y)
        feed = self.create_feed_dict(inputs_batch= small_s, labels_batch= small_s_y, dropout= self.config.drop_out)
    ...
    def add_embeddings(self): #use this function to convert inputs to a tensor of shape (none, sentence length (self.config.max_length), number of features * embedding size (self.config.n_features * self.config.embed_size)
    ...
    def return_one_hot(self, ind):
        zero = np.zeros(self.config.output_size)
        zero[ind] = 0
        return zero
    ...
    def run_epoch(self, sess, parser, dev_set):

        for i, inds in enumerate(get_batch(self.data_size, self.config.batch_size)): #inds are indices for the training example

            batch = self.training_set[inds]
    ...
    def prediction(self): #main training function for the model
        ...
        self.one_hot_index()
        self.set_cell()
        state = self.cell.zero_state(self.config.batch_size, dtype = tf.float32) #initial state

        outputs, states = tf.nn.dynamic_rnn(self.cell, self.x, sequence_length= self.sequence_lengths, initial_state=state, time_major=True)
        ...
        for time_step in range(self.config.max_length-2):
...
c = Config(max_length = max_length, embed_size = embeddings.shape[1], output_size=embeddings.shape[0])
